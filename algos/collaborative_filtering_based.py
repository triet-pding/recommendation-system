...
from typing import List, Tuple, Union, Dict, Optional, Any, Set
import pickle
import structlog
...
class ItemBasedCFRecommender:
    ...
    def __init__(self, config: RecommendationConfig):
        ...
        self.model_dir = Path(config.model_dir) / "cf_models"
        ...
        logger.info(f"--- Collaborative Filtering Recommender initialized ---\n")
    ...
    def fit(self, ratings_df: pd.DataFrame, save_model: bool = True):
        ...
        """
        logger.info("Starting collaborative filtering model fitting...")
    ...
    def _compute_popular_items(self):
        ...
        try:
            popular_items = self.recommend_popular_items_wilson(n_recommendations=20)
    ...
    def recommend_for_user(self, user_id: str, n_recommendations: int = 20, 
    ...
    def _recommend_for_user(self, user_id: str, n_recommendations: int = 10, 
        ...
        # If not found user, generate pre-computed popular item instead
        ...
        # Get user's ratings and compute user's average rating
        watched_items = set(user_vector.indices.tolist())
        watched_ratings = user_vector.data
        user_mean_rating = np.mean(watched_ratings)
        
        # Use rating deviation from user mean
        scores = defaultdict(float)
        total_similarity = defaultdict(float)
        
        # Create rating deviations for better prediction
        item_ratings = {}
        for idx, item_idx in enumerate(user_vector.indices):
            rating = watched_ratings[idx]
            # Store both rating and deviation from user mean
            item_ratings[item_idx] = {
                'rating': rating,
                'deviation': rating - user_mean_rating
            }
        
        for item_idx, rating_info in item_ratings.items():
            # Dynamic rating threshold based on user's rating pattern
            rating_threshold = max(3.0, user_mean_rating - 0.5) # can make this as dynamic
            if rating_info['rating'] < rating_threshold:
                continue
            
            if item_idx in self.item_similarity_matrix:
                # Only consider top-k similar items (reduces noise)
                similar_items = sorted(
                    self.item_similarity_matrix[item_idx].items(),
                    key=lambda x: x[1], reverse=True
                )[:n_recommendations]  # Top-n similar items
                ...
                for similar_item, similarity in similar_items:
                    ...
                    # Use rating deviation for better prediction
                    predicted_rating = user_mean_rating + (rating_info['deviation'] * similarity)
                    
                    scores[similar_item] += predicted_rating * similarity
                    total_similarity[similar_item] += abs(similarity)
        
        # Normalize scores properly
        normalized_scores = {}
        for item_idx, score in scores.items():
            if total_similarity[item_idx] > 0:
                normalized_score = score / total_similarity[item_idx]
                # Ensure score is within reasonable bounds
                normalized_scores[item_idx] = max(1.0, min(5.0, normalized_score))
        
        # Diversity boost for recommendations
        if len(normalized_scores) > n_recommendations * 2:
            # Apply diversity - select from different score ranges
            score_ranges = self._create_score_ranges(normalized_scores, n_recommendations)
            diverse_items = self._select_diverse_items(score_ranges, n_recommendations)
            top_items = diverse_items
        else:
            available_items = min(len(normalized_scores), n_recommendations)
            top_items = heapq.nlargest(available_items, normalized_scores.keys(), 
        ...
        # Fill with popular items using smart blending
        if len(recommendations) < n_recommendations:
            recommendations = self._blend_with_popular_items(
                recommendations, n_recommendations, watched_items
            )
        
        return recommendations

    def _create_score_ranges(self, scores_dict: Dict[str, float], n_recommendations: int) -> Dict[str, Any]:
        """Create score ranges for diversity selection based on n_recommendations"""
        
        if len(scores_dict) <= n_recommendations:
            # If we have fewer candidates than needed, return all in high range
            return {
                'high': scores_dict,
                'medium': {},
                'low': {}
        ...
        # Adaptive thresholds based on n_recommendations
        if n_recommendations <= 5:
            # For small recommendation sets, be more selective
            high_threshold = np.percentile(sorted_scores, 90)
            medium_threshold = np.percentile(sorted_scores, 70)
        elif n_recommendations <= 10:
            # Standard thresholds
            high_threshold = np.percentile(sorted_scores, 80)
            medium_threshold = np.percentile(sorted_scores, 50)
        else:
            # For larger sets, include more diversity
            high_threshold = np.percentile(sorted_scores, 70)
            medium_threshold = np.percentile(sorted_scores, 40)
        
        # Ensure minimum items in each range
        min_items_per_range = max(1, n_recommendations // 5)  # At least 20% of n_recommendations
        
        high_items = {k: v for k, v in scores_dict.items() if v >= high_threshold}
        medium_items = {k: v for k, v in scores_dict.items() if medium_threshold <= v < high_threshold}
        low_items = {k: v for k, v in scores_dict.items() if v < medium_threshold}
        
        # Rebalance if ranges are too small
        if len(high_items) < min_items_per_range and len(medium_items) > 0:
            # Move some medium items to high
            sorted_medium = sorted(medium_items.items(), key=lambda x: x[1], reverse=True)
            items_to_move = min(min_items_per_range - len(high_items), len(sorted_medium))
            
            for item_id, score in sorted_medium[:items_to_move]:
                high_items[item_id] = score
                del medium_items[item_id]
        ...
        return {
            ...
            'metadata': {
                ...
                'total_candidates': len(scores_dict)
            }
        }

    def _select_diverse_items(self, score_ranges: Dict[str, Any], n_recommendations: int) -> List[str]:
        """Select items from different score ranges for diversity"""
        # Allocate recommendations: 60% high, 30% medium, 10% low
        high_count = int(n_recommendations * 0.6)
        medium_count = int(n_recommendations * 0.3)
        low_count = n_recommendations - high_count - medium_count
        
        selected_items = []
        
        # Select from high range
        high_items = heapq.nlargest(high_count, score_ranges['high'].keys(),
                                key=score_ranges['high'].get)
        selected_items.extend(high_items)
        
        # Select from medium range
        medium_items = heapq.nlargest(medium_count, score_ranges['medium'].keys(),
                                    key=score_ranges['medium'].get)
        selected_items.extend(medium_items)
        
        # Select from low range
        low_items = heapq.nlargest(low_count, score_ranges['low'].keys(),
                                key=score_ranges['low'].get)
        selected_items.extend(low_items)
        
        return selected_items

    def _blend_with_popular_items(self, recommendations: List[str], n_recommendations: int, watched_items: Set) -> List[str]:
        """Intelligently blend with popular items"""
        remaining_slots = n_recommendations - len(recommendations)
        recommended_item_ids = {rec[0] for rec in recommendations}
        
        # Get popular items not already recommended or watched
        popular_candidates = []
        for item_id, popularity_score in self.popular_items_cache:
            if (item_id not in recommended_item_ids and 
                self.item_mapping.get(item_id) not in watched_items):
                popular_candidates.append((item_id, popularity_score))
        
        # Add popular items with decaying scores to maintain ranking
        decay_factor = 0.8
        for i, (item_id, score) in enumerate(popular_candidates[:remaining_slots]):
            decayed_score = score * (decay_factor ** (i + 1))
            recommendations.append((item_id, decayed_score))
    ...
    def load_model(self, filepath: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.info(f"--- Collaborative model loaded from {filepath} ---")
    ...
    def evaluate(self, test_data: pd.DataFrame, k: Union[int, List[int]] = 10, metrics: List[str] = None, 
             relevance_threshold: float = 3.0, max_users_for_coverage: int = 100) -> Dict[str, float]:
        """
        ...
        logger.info(f"Evaluating collaborative filtering recommender on {len(test_data)} test ratings...")
                ...
                for user_id, relevant_items in user_test_items.items():
                    try:
                        ...
                        try:
                            ...
                            if not recs:
                                logger.warning(f"Warning: No recommendations generated for user {user_id}")
                                failed_recommendations += 1
                                continue
                                
                            # Log if we got fewer than expected
                            if len(recs) < max_k:
                                logger.warning(f"Warning: Only got {len(recs)} recommendations for user {user_id} (requested {max_k})")
                            
                        except Exception as e:
                            logger.error(f"Exception for user {user_id}: {e}")
                            failed_recommendations += 1
                            continue
